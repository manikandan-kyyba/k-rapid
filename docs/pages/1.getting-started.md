...
```sh
yarn add react-native-kyba
```
or
```sh
npm install react-native-kyba
```
...
module.exports = {
  ...
  env: {
    production: {
      plugins: ['react-native-kyba/babel'],
    },
  },
};
```
...
Wrap your root component in `Provider` from `react-native-kyba`. If you have a vanilla React Native project, it's a good idea to add it in the component which is passed to `AppRegistry.registerComponent`. This will usually be in the `index.js` file. If you have an Expo project, you can do this inside the exported component in the `App.js` file.
