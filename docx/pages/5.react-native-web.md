...
Make sure that you have followed the getting started guide and have `react-native-kyyba` installed and configured before following this guide.
...
Next, we want to tell `babel-loader` to compile `react-native-kyyba` and `react-native-vector-icons`. We would also want to disable reading the babel configuration files to prevent any conflicts.
